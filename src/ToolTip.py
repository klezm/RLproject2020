...
class ToolTip:
    ...
    def leave(self):
        self.widget.winfo_toplevel().deiconify()  # Prevents the toplevel from iconifying if the curser left the widget directly via the toplevel border
        self.tipWindow.withdraw()
        if self.afterId is not None:
            self.afterId = self.widget.after_cancel(self.afterId)
